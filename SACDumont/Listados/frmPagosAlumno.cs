...
{
    public partial class frmPagosAlumno : frmBaseConsulta
    {
        #region Variables
        ComboAlumnos txtBuscar = new ComboAlumnos();
        ...
        private void CargarMenu()
        {
            ...
            tsSearch.Height = 32; // ajusta si tu barra es más alta

            // 2) Crea tu control y dale tamaño visible
            txtBuscar = new ComboAlumnos();
            txtBuscar.Size = new Size(220, 24);   // ¡no lo dejes en (0,0)!
            txtBuscar.Margin = new Padding(4, 4, 4, 4);

            // 3) Host explícito con AutoSize desactivado
            var host = new ToolStripControlHost(txtBuscar)
            {
                AutoSize = false,
                Size = txtBuscar.Size,
            ...
            // 4) (Opcional) engancha tu evento personalizado
            txtBuscar.OnAlumnoSeleccionado += txtBuscar_OnAlumnoSeleccionado;

            // 5) Asegúrate de agregarlo al ToolStrip correcto
            tsSearch.Items.Add(host);
        ...
        private void txtBuscar_OnAlumnoSeleccionado(DataRow obj)
        {
            idAlumno = Convert.ToInt32(obj["matricula"]);
            CargarDatos();
        }
        ...
        private void CargarDatos()
        {
            dtPagosAl = sqlServer.ExecSQLReturnDT($@"SELECT m.id_movimiento ,m.fechahora AS Fecha, p.descripcion AS Producto, a.appaterno + ' ' + a.apmaterno + ' ' + a.nombre AS Alumno, cat.descripcion AS Grado, catG.descripcion AS Grupo, 
                                                        ...
                                                        WHERE m.id_ciclo = {basGlobals.iCiclo} AND m.id_matricula = {idAlumno};", "Movimientos");

            bs.DataSource = dtPagosAl;
            ...
            //{
            //    var listaGeneral = db.Movimientos.Where(m => m.id_ciclo == basGlobals.iCiclo && m.id_matricula == idAlumno).Include(mc => mc.MovimientosCobros)
            //        .Include(mp => mp.MovimientosProductos).ToList();
            //}
        }
        ...
        private void frmPagosAlumno_Resize(object sender, EventArgs e)
        {
